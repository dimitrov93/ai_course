...
async function main() {
  const movies = loadJSONData<{ name: string; description: string }[]>('project/movies.json');
  const data = movies.map((m) => `${m.name}: ${m.description}`);
  ...
  saveDataToJsonFile(dataWithEmbeddings, 'movieEmbedings.json');
}
